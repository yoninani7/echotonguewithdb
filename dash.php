...
function generate_token() { if (empty($_SESSION['csrf_token'])) { $_SESSION['csrf_token'] = bin2hex(random_bytes(32)); } return $_SESSION['csrf_token']; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header("Location: ?tab=$tab_redirect"); exit;
}
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;700&family=Space+Grotesk:wght@400;700&display=swap" rel="stylesheet">
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; outline: none; }
        ...
        /* Sidebar */
        .sidebar { width: 280px; background: var(--bg-sidebar); border-right: var(--border); display: flex; flex-direction: column; padding: 25px; position: fixed; height: 100vh; z-index: 100; transition: 0.3s; }
        .brand { font-family: 'Cinzel Decorative', sans-serif; font-size: 26px; font-weight: 700; color: #fff;  display: flex; align-items: center; gap: 10px; text-decoration: none; margin: 0 auto;margin-bottom: 40px; }
        ...
        .nav-item { padding: 14px 18px; border-radius: 12px; color: var(--text-muted); text-decoration: none; font-weight: 500; display: flex; align-items: center; gap: 14px; transition: 0.2s; }
        ...
        /* Main Content */
        .main { flex: 1; margin-left: 280px; padding: 30px 40px; width: calc(100% - 280px); }
        ...
        .btn {  padding: 12px 25px;
            ...
            overflow: hidden;}
        .btn-primary { 
            background: radial-gradient(100% 120% at 50% 0%, #df0211 0%, #500D11 100%); color: white;   }
        .btn-primary:hover {  background: radial-gradient(100% 120% at 50% 0%, rgba(255, 12, 29, 0.95) 0%, #500D11 100%);  transform: translateY(-1px); }
        
        /* Disabled Button Style */
        .btn:disabled, .btn-disabled { background: #333; color: #777; cursor: not-allowed; box-shadow: none; opacity: 0.6; pointer-events: none; }
        ...
        /* Textarea */
        .thought-input { width: 100%; background: #0a0a0a; border: var(--border); color: #fff; padding: 15px; border-radius: 12px; font-family: inherit; font-size: 15px; resize: vertical; min-height: 120px; margin-bottom: 15px; transition: 0.3s; }
        .thought-input:focus { border-color: var(--primary); }
        ...
        .page-link { width: 35px; height: 35px; display: flex; align-items: center; justify-content: center; background: rgba(255,255,255,0.05); border-radius: 8px; color: #fff; text-decoration: none; }
        .page-link.active { background: var(--primary); }

        /* Toast */
        .toast { position: fixed; top: 20px; right: 20px; background: #151515; border-left: 4px solid var(--primary); padding: 15px 25px; border-radius: 8px; box-shadow: 0 10px 30px rgba(0,0,0,0.5); transform: translateX(120%); transition: 0.3s cubic-bezier(0.68, -0.55, 0.27, 1.55); z-index: 2000; display: flex; align-items: center; gap: 15px; }
...
<main class="main">
    ...
    <div class="top-bar"> 
        <div style="display:flex; align-items:center; gap:15px;">
            ...
            <h2 style="font-weight:700; font-family: 'Cinzel Decorative', sans-serif"><?php echo ucfirst($active_tab); ?></h2>
        </div>
    </div>
    ...
    <div class="stats-grid">
        <div class="stat-card">
            ...
            <div class="stat-info"><h3><?php echo $stats['thoughts']; ?></h3><p>Total Thoughts</p></div>
        </div>
        <div class="stat-card">
            <div class="stat-icon"><i class="fas fa-users"></i></div>
            <div class="stat-info"><h3><?php echo $stats['subs']; ?></h3><p>Subscribers</p></div>
        </div>
        <div class="stat-card">
            <div class="stat-icon"><i class="fas fa-star"></i></div>
            <div class="stat-info"><h3><?php echo number_format($stats['avg_rating'], 1); ?></h3><p>Avg Rating</p></div>
        </div>
        <div class="stat-card">
            <div class="stat-icon"><i class="fas fa-envelope"></i></div>
            <div class="stat-info"><h3><?php echo $stats['unread']; ?></h3><p>Unread Feedback</p></div>
        </div>
    </div>
        ...
        <div class="glass-card">
            <form method="POST">
                ...
                <div style="display:flex; gap:12px; justify-content:flex-end;">
                    <button type="button" class="btn btn-secondary" onclick="document.getElementById('newThoughtText').value = ''">
        ...
        <div class="glass-card" style="padding:0">
            ...
            <?php else: ?>
                <div class="table-container">
                    <table>
                        <thead><tr><th>Published Date</th><th>Content</th><th style="text-align:right">Actions</th></tr></thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?php echo nl2br(substr(sanitize($row['thought_text']), 0, 100)) . (strlen($row['thought_text']) > 100 ? '...' : ''); ?></td>
                                <td style="text-align:right">
                                    <button class="btn btn-icon" onclick="openModal('edit', <?php echo htmlspecialchars(json_encode($row)); ?>)"><i class="fas fa-edit"></i></button>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="text-align:right">
                                    <button class="btn btn-icon" onclick="openModal('view', <?php echo htmlspecialchars(json_encode($row)); ?>)"><i class="fas fa-eye"></i></button>
    ...
    <?php if ($active_tab === 'newsletter'): ?>
        <div style="display:flex; justify-content:flex-end; margin-bottom:20px;">
            <form method="POST">
                ...
                <?php else: ?>
                    <button type="button" class="btn btn-disabled" disabled title="No subscribers to export"><i class="fas fa-download"></i> Export CSV</button>
                <?php endif; ?>
            </form>
        </div>
        ...
        <div class="glass-card" style="padding:0">
            ...
            <?php else: ?>
                <div class="table-container">
                    <table>
                        <thead><tr><th>ID</th><th>Email</th><th>Subscribed</th><th style="text-align:right">Actions</th></tr></thead>
                        <tbody>
                            <?php foreach($data as $row): ?>
                            <tr>
...
<div id="editModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.8); backdrop-filter:blur(5px); z-index:1000; align-items:center; justify-content:center;">
    <div class="glass-card" style="width:90%; max-width:500px; background:#111;">
        ...
        <form method="POST">
            ...
            <div style="display:flex; justify-content:flex-end; gap:10px; margin-top:15px">
                <button type="button" class="btn" style="background:#222; color:#fff" onclick="document.getElementById('editModal').style.display='none'">Cancel</button>
...
<div id="viewModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.8); backdrop-filter:blur(5px); z-index:1000; align-items:center; justify-content:center;">
    <div class="glass-card" style="width:90%; max-width:600px; background:#111;">
        ...
        <button type="button" class="btn" style="background:#222; color:#fff; float:right" onclick="document.getElementById('viewModal').style.display='none'">Close</button>
    </div>
</div>
